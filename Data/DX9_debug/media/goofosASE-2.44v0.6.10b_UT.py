...
"""
Name: 'ASCII Scene (.ase) v0.6.10 (unreal)'
Blender: 244
Group: 'Export'
Tooltip: 'ASCII Scene Export (*.ase) (unreal)'
"""
__author__ = "Goofos"
__version__ = "0.6.10"
...
def mesh_faceList(file, idnt, me, materials, sGroups, faces, matTable, hasTable, count):
   ...
   smoothgroups = find_smoothgroups(faces)
   for current_face in faces:

      face_verts = current_face.verts
      smooth = '*MESH_SMOOTHING %d' % smoothgroups[current_face]
      matID = '*MESH_MTLID 0'
      ...
      if len(face_verts) is 3:
         ...
         #   else:
         #      smooth += ' 1'
         #      print 'Failed: ', faceNo, smooth ##PHL
         #
         #elif current_face.smooth:
         #   smooth += ' 1'
         #   print 'Found a smooth face: ', faceNo
         #   
         ###PHL
         #if sGroups.has_key(vert0) and sGroups.has_key(vert1) and sGroups.has_key(vert2) and sGroups.has_key(vert3):
         #   print 'Hmmm', faceNo, current_face.smooth
         ### end PHL
...
def find_smoothgroups(faces):
   """Find smoothgroups for a collection of faces.
   
   Returns a dictionary where the key is a face (in faces) and the value is an
   integer containing the smoothgroup.
   
   If a face shares a vertex with another face, it is assumed to be in the same
   smoothgroup. Vertexes with the same coordinates can belong to different
   smoothgroups (for example, splitting a face off in Blender with Y-KEY
   will result in vertexes with the same coordinates. But they would not be
   in the same smoothgroup because they have disinct vertices.)
   """
   # submeshes will contain lists. Each list represesnts a smoothgroup
   submeshes = []
   face_dict = {} # dict we will be returning
   for face in faces:
      v0 = face.verts[0].index
      v1 = face.verts[1].index
      v2 = face.verts[2].index
      if len(face.verts) == 4:
         v3 = face.verts[3].index
         new_verts = (v0, v1, v2, v3)
      else:
         new_verts = (v0, v1, v2)
   
      new_mesh_group = []
      smoothgroup = 1
      # Loop through all our submeshes we have already processed, looking
      # for any vertices that are in our new_verts group. If we find a match
      # add it to the submesh (representing a smoothgroup).
      for sm in submeshes:
         for verts in sm:
            for vert in new_verts:
               if vert in verts:
                  new_mesh_group = sm
                  new_mesh_group.append(new_verts)
                  break
            if new_mesh_group:
               break
         if new_mesh_group:
      ...
      # We didn't find a match, so this must be the first member of a new
      # smoothgroup. Add it as a new member to submeshes.
      if not new_mesh_group:
         submeshes.append([new_verts])
         
      face_dict[face] = smoothgroup
      # print "face_dict[face] = %d" % face_dict[face]
   return face_dict
...
def phl():
   mesh = Scene.GetCurrent().getActiveObject()
   print mesh
...
def write_ui(filename):
   ...
   # Get USER Options
   pup_block = [('Mesh Options...'),
               ('Apply Modifiers', EXPORT_MOD, 'Use modified mesh data from each object.'),
               ('Materials', EXPORT_MTL, 'Export Materials.'),
               ('Face UV', EXPORT_UV, 'Export texface UV coords.'),
               ('Vertex Colors', EXPORT_VC, 'Export vertex colors'),
               ('Context...'),
               ('Selection Only', EXPORT_SELO, 'Only export objects in visible selection, else export all mesh object.'),
               ('Bonus...'),
               #('VertGr. as SmoothGr.', EXPORT_VG2SG, 'Make SmoothGroups by VertGroups. See doc.'),
               ('Center Objects', EXPORT_REC, 'Center ALL objects to World-Grid-Origin-Center-Point-(0,0,0). ;)')]
